...
```bash
node ~/.claude/skills/excalidraw-diagram/references/render_excalidraw.mjs <path-to-file.excalidraw>
```

(Project-local install? Then it's `.claude/skills/excalidraw-diagram/references/render_excalidraw.mjs`.)

This outputs a PNG next to the `.excalidraw` file. Then use the **Read tool** on the PNG to actually view it.

**It borrows a Playwright rather than shipping one.** Resolution order: `$EXCALIDRAW_PLAYWRIGHT` → the skill's own `references/node_modules` → **the current repo's `node_modules`** → bare resolution. Inside any repo that already has Node Playwright, there is nothing to set up. Browsers are installed per-user, so one `npx playwright install chromium` covers every repo and this skill.
...
Usually none — the script borrows the Playwright in whatever repo you're standing in. If it reports that it can't find one:

```bash
npx playwright install chromium          # once per machine (shared browser cache)
